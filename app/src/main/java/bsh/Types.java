...
class Types {
    ...
    */
    ...
    /**
     * Special value that indicates by identity that the result of a cast operation was a valid
     * cast. This is used by castObject() and castPrimitive() in the checkOnly mode of operation.
     * This value is a Primitive type so that it can be returned by castPrimitive.
     */
    ...
    /**
     * Is the 'from' signature (argument types) assignable to the 'to' signature (candidate method
     * types) This method handles the special case of null values in 'to' types indicating a loose
     * type and matching anything.
     */
    ...
    static boolean isSignatureAssignable(Class[] from, Class[] to, int round) {
        ...
        switch (round) {
            ...
            case BSH_ASSIGNABLE:
                ...
                return true;
            default:
                throw new InterpreterError("大小写错误");
        }
    }

    /**
     * Are the two signatures exactly equal? This is checked for a special case in overload
     * resolution.
     */
    static boolean areSignaturesEqual(Class[] from, Class[] to) {
        if (from.length != to.length) return false;

        for (int i = 0; i < from.length; i++) if (from[i] != to[i]) return false;
    ...
    /**
     * Test if a type can be converted to another type via BeanShell extended syntax rules (a
     * superset of Java conversion rules).
     */
    static boolean isBshAssignable(Class toType, Class fromType) {
        try {
            return castObject(toType, fromType, null /*fromValue*/, ASSIGNMENT, true) == VALID_CAST;
        } catch (UtilEvalError e) {
            // This should not happen with checkOnly true
            throw new InterpreterError("投映测试失败: " + e);
        }
    }
    ...
    public static Object castObject(Object fromValue, Class toType, int operation)
        ...
        if (fromValue == null) throw new InterpreterError("来源值为空");
    ...
    */
    private static Object castObject(
            Class toType, Class fromType, Object fromValue, int operation, boolean checkOnly)
            throws UtilEvalError {
        /*
        ...
        */
        if (checkOnly && fromValue != null) throw new InterpreterError("投映参数1错误");
        if (!checkOnly && fromValue == null) throw new InterpreterError("投映参数2错误");
        if (fromType == Primitive.class) throw new InterpreterError("来源值错误, 需要拆包");
        if (fromValue == Primitive.NULL && fromType != null) throw new InterpreterError("参数1不一致");
        if (fromValue == Primitive.VOID && fromType != Void.TYPE)
            throw new InterpreterError("参数2不一致");
        if (toType == Void.TYPE) throw new InterpreterError("松散的目标值应该是空");
        ...
        if (toType.isPrimitive()) {
            if (fromType == Void.TYPE || fromType == null || fromType.isPrimitive()) {
                ...
                if (Primitive.isWrapperType(fromType)) {
                    ...
                    else primFromValue = (Primitive) Primitive.wrap(fromValue, unboxedFromType);

                    return Primitive.castPrimitive(
                            toType, unboxedFromType, primFromValue, checkOnly, operation);
                } else {
                    // Cannot cast from arbitrary object to primitive
                    if (checkOnly) return INVALID_CAST;
                    else throw castError(toType, fromType, operation);
                }
            }
        }
    ...
    /**
     * Return a UtilEvalError or UtilTargetError wrapping a ClassCastException describing an illegal
     * assignment or illegal cast, respectively.
     */
    static UtilEvalError castError(Class lhsType, Class rhsType, int operation) {
        return castError(
                Reflect.normalizeClassName(lhsType),
                Reflect.normalizeClassName(rhsType),
                operation);
    }

    static UtilEvalError castError(String lhs, String rhs, int operation) {
        if (operation == ASSIGNMENT) return new UtilEvalError("不能声明 " + rhs + " 为 " + lhs);

        Exception cce = new ClassCastException("不能投射 " + rhs + " 到 " + lhs);
