...
/**
 * A BshClassPath encapsulates knowledge about a class path of URLs. It can maps all classes the
 * path which may include: jar/zip files and base dirs
 *
 * <p>A BshClassPath may composite other BshClassPaths as components of its path and will reflect
 * changes in those components through its methods and listener interface.
 *
 * <p>Classpath traversal is done lazily when a call is made to getClassesForPackage() or
 * getClassSource() or can be done explicitily through insureInitialized(). Feedback on mapping
 * progress is provided through the MappingFeedback interface.
 *
 * <p>Design notes: Several times here we traverse ourselves and our component paths to produce a
 * composite view of some thing relating to the path. This would be an opportunity for a visitor
 * pattern.
 */
public class BshClassPath implements ClassPathListener, NameSource {
    ...
    /**
     * Add the specified BshClassPath as a component of our path. Changes in the bcp will be
     * reflected through us.
     */
    public void addComponent(BshClassPath bcp) {
        if (compPaths == null) compPaths = new ArrayList();
        compPaths.add(bcp);
    ...
    public void add(URL[] urls) {
        ...
        if (mapsInitialized) map(urls);
    }

    public void add(URL url) throws IOException {
        path.add(url);
        if (mapsInitialized) map(url);
    }
    ...
    /**
     * Explicitly set a class source. This is used for generated classes, but could potentially be
     * used to allow a user to override which version of a class from the classpath is located.
     */
    ...
    /**
     * If the claspath map is not initialized, do it now. If component maps are not do them as
     * well...
     *
     * <p>Random note: Should this be "insure" or "ensure". I know I've seen "ensure" used in the
     * JDK source. Here's what Webster has to say:
     *
     * <p>Main Entry:ensure Pronunciation:in-'shur Function:transitive verb Inflected
     * Form(s):ensured; ensuring : to make sure, certain, or safe : GUARANTEE synonyms ENSURE,
     * INSURE, ASSURE, SECURE mean to make a thing or person sure. ENSURE, INSURE, and ASSURE are
     * interchangeable in many contexts where they indicate the making certain or inevitable of an
     * outcome, but INSURE sometimes stresses the taking of necessary measures beforehand, and
     * ASSURE distinctively implies the removal of doubt and suspense from a person's mind. SECURE
     * implies action taken to guard against attack or loss.
     */
    ...
    /**
     * @param topPath indicates that this is the top level classpath component and it should send
     *     the startClassMapping message
     */
    protected synchronized void insureInitialized(boolean topPath) {
        ...
        if (topPath && !mapsInitialized) startClassMapping();
        ...
        // initialize ourself
        if (!mapsInitialized) map((URL[]) path.toArray(new URL[0]));

        if (topPath && !mapsInitialized) endClassMapping();

        mapsInitialized = true;
    }

    /**
     * Get the full path including component paths. (component paths listed first, in order)
    ...
    /**
     * Support for super import "*"; Get the full name associated with the unqualified name in this
     * classpath. Returns either the String name or an AmbiguousName object encapsulating the
     * various names.
     */
    public String getClassNameByUnqName(String name) throws ClassPathException {
        ...
        if (obj instanceof AmbiguousName)
            throw new ClassPathException("含糊不清的类名: " + ((AmbiguousName) obj).get());
    ...
    */
    private UnqualifiedNameTable getUnqualifiedNameTable() {
        if (unqNameTable == null) unqNameTable = buildUnqualifiedNameTable();
        return unqNameTable;
    }
    ...
    public String[] getAllNames() {
        ...
        if (nameCompletionIncludesUnqNames) names.addAll(getUnqualifiedNameTable().keySet());
    ...
    synchronized void map(URL[] urls) {
        for (int i = 0; i < urls.length; i++)
            try {
                ...
                String s = "架构classpath时出错: " + urls[i] + ": " + e;
                errorWhileMapping(s);
            }
    }
    ...
    private void mapClass(String className, Object source) {
        ...
        if (obj == null) classSource.put(className, source);
    }
    ...
    */
    ...
    static List traverseDirForClassesAux(File topDir, File dir) throws IOException {
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            if (child.isDirectory()) list.addAll(traverseDirForClassesAux(topDir, child));
            else {
                ...
                if (isClassFileName(name)) {
                    ...
                    package-class part
                    */
                    if (name.startsWith(top)) name = name.substring(top.length() + 1);
                    else throw new IOException("路径解析失败");
    ...
    static String[] searchJarForClasses(URL jar) throws IOException {
        ...
        while ((ze = zin.getNextEntry()) != null) {
            ...
            if (isClassFileName(name)) v.addElement(canonicalizeClassName(name));
        }
        zin.close();
    ...
    /**
     * Create a proper class name from a messy thing. Turn / or \ into ., remove leading class and
     * trailing .class
     *
     * <p>Note: this makes lots of strings... could be faster.
     */
    ...
    public static URL[] getUserClassPathComponents() throws ClassPathException {
        if (userClassPathComp != null) return userClassPathComp;
        ...
        try {
            ...
            throw new ClassPathException("解析类路径失败: " + e);
        }
    ...
    public static BshClassPath getUserClassPath() throws ClassPathException {
        if (userClassPath == null)
            userClassPath = new BshClassPath("用户类路径", getUserClassPathComponents());
    ...
    public static BshClassPath getBootClassPath() throws ClassPathException {
        if (bootClassPath == null) {
            try {
                ...
                bootClassPath = new BshClassPath("启动类路径", new URL[] {url});
            } catch (MalformedURLException e) {
                throw new ClassPathException("找不到启动jar: " + e);
            }
        }
    ...
    private static String getRTJarPath() {
        ...
        if (!urlString.startsWith("jar:file:")) return null;
    ...
    public static class DirClassSource extends ClassSource {
        ...
        public static byte[] readBytesFromFile(File base, String className) {
            ...
            try {
                ...
                throw new RuntimeException("不能加载文件 " + file);
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        BshClassPath bcp = new BshClassPath("测试", urls);
    }
    ...
    */
    static class UnqualifiedNameTable extends HashMap {
        void add(String fullname) {
            ...
            if (have == null) super.put(name, fullname);
            else if (have instanceof AmbiguousName) ((AmbiguousName) have).add(fullname);
            else { // String
    ...
    void nameSpaceChanged() {
        if (nameSourceListeners == null) return;
    ...
    public void addNameSourceListener(NameSource.Listener listener) {
        if (nameSourceListeners == null) nameSourceListeners = new ArrayList();
    ...
    public static void addMappingFeedback(MappingFeedback mf) {
        if (mappingFeedbackListener != null) throw new RuntimeException("未实现! 已经是一个监听器");
        mappingFeedbackListener = mf;
    }

    void startClassMapping() {
        if (mappingFeedbackListener != null) mappingFeedbackListener.startClassMapping();
        else System.err.println("开始类路径映射");
    }

    void classMapping(String msg) {
        if (mappingFeedbackListener != null) {
            mappingFeedbackListener.classMapping(msg);
        } else System.err.println("映射: " + msg);
    }

    void errorWhileMapping(String s) {
        if (mappingFeedbackListener != null) mappingFeedbackListener.errorWhileMapping(s);
        else System.err.println(s);
    }

    void endClassMapping() {
        if (mappingFeedbackListener != null) mappingFeedbackListener.endClassMapping();
        else System.err.println("停止类路径映射");
    }
    ...
    public static interface MappingFeedback {
         ...
         *     classMapping( String msg, int perc );
         */
