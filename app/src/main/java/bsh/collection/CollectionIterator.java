 ...
 * collections API supported by JDK1.2 and greater.
 *
 * @author Daniel Leuck
 * @author Pat Niemeyer
 */
public class CollectionIterator implements bsh.BshIterator {
    ...
    protected Iterator createIterator(Object iterateOverMe)
        ...
        if (iterateOverMe == null)
            throw new NullPointerException("传递到CollectionIterator架构器的对象不能是空的");

        if (iterateOverMe instanceof Iterator) return (Iterator) iterateOverMe;

        if (iterateOverMe instanceof Collection) return ((Collection) iterateOverMe).iterator();

        Iterator it = getIteratorForIterable(iterateOverMe);
        if (it != null) return it;

        final CollectionManager.BasicBshIterator bbi =
                new CollectionManager.BasicBshIterator(iterateOverMe);
    ...
    /**
     * Get an Iterator for a Java 5 Iterable object. Rather than resorting to another loadable
     * module for Java 5 we'll use reflection here to invoke the Iterable iterator() method.
     *
