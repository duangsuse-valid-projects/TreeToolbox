...
public class NameSpace implements java.io.Serializable, BshClassManager.Listener, NameSource {
    ...
    */
     ...
     * imported). Qualified names are always absolute and are cached by BshClassManager.
     */
    ...
    Variable setVariable(String name, Object value, boolean strictJava, boolean recurse)
        ...
        // No previous variable definition found here (or above if recurse)
        {
            ...
            Variable var = createVariable(name, value, null);
     ...
     * Finally, if neither the variable or the property exist, we create a new variable.
     */
    void setLocalVariableOrProperty(String name, Object value, boolean strictJava)
            throws UtilEvalError {
        setVariableOrProperty(name, value, strictJava, false);
    }
    ...
    void setVariableOrProperty(String name, Object value, boolean strictJava, boolean recurse)
        ...
        // No previous variable definition found here (or above if recurse)
        {
            ...
            varScope.variables.put(name, createVariable(name, value, null));
    ...
    protected Variable createVariable(String name, Object value, Modifiers mods)
            throws UtilEvalError {
    ...
    /**
     * Get the parent namespace. Note: this isn't quite the same as getSuper(). getSuper() returns
     * 'this' if we are at the root namespace.
     */
    ...
    */
    ...
    /**
     * Declare a variable in the local scope and set its initial value. Value may be null to
     * indicate that we would like the default value for the variable type. (e.g. 0 for integer
     ...
     * importCommands("bsh.commands") are equivalent. If a relative path style specifier is used
     * then it is made into an absolute path by prepending "/".
     */
    ...
    protected BshMethod getImportedMethod(String name, Class[] sig) throws UtilEvalError {
        ...
        if (importedObjects != null)
            for (int i = 0; i < importedObjects.size(); i++) {
                ...
                Method method =
                        Reflect.resolveJavaMethod(getClassManager(), clas, name, sig, false);
    ...
    */
    ...
    /**
     * Load a class through this namespace taking into account imports. The class search will
     * proceed through the parent namespaces if necessary.
     *
    ...
    /**
     * Try to make the name into an imported class. This method takes into account only imports
     * (class or package) found directly in this NameSpace (no parent chain).
     */
    ...
    */
    ...
    public void loadDefaultImports() {
        /**
         * Note: the resolver looks through these in reverse order, per precedence rules... so for
         * max efficiency put the most common ones later.
         */
        importClass("bsh.EvalError");
        importClass("bsh.Interpreter");
        importPackage("javax.swing.event");
    ...
    /**
     * Import a compiled Java object's methods and variables into this namespace. When no scripted
    ...
    */
    ...
    /**
     * If a writable property exists for the given name, set it and return true, otherwise do
     * nothing and return false.
     */
    boolean attemptSetPropertyValue(String propName, Object value, Interpreter interp)
            throws UtilEvalError {
