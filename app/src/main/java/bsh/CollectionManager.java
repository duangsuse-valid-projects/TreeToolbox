...
/**
 * The default CollectionManager (which remains Java 1.1 compatible) supports iteration over objects
 * of type: Enumeration, Vector, String, StringBuffer and array. The dynamically loaded
 * CollectionManagerImpl supports additional types when it is present.
 *
...
public class CollectionManager {
    ...
    public static synchronized CollectionManager getCollectionManager() {
        ...
        if (manager == null) manager = new CollectionManager(); // default impl
    ...
    /**
     * Determine dynamically if the target is an iterator by the presence of a pair of next() and
    ...
    public static class BasicBshIterator implements BshIterator {
        ...
        protected Enumeration createEnumeration(Object iterateOverMe) {
            ...
            if (iterateOverMe instanceof Vector) return ((Vector) iterateOverMe).elements();
