...
public class BshClassManager {
    ...
    /**
     * Global cache for things we know are *not* classes. Note: these should probably be
    ...
    /**
     * Load the specified class by name, taking into account added classpath and reloaded classes,
     * etc. Note: Again, this is just a trivial implementation. See bsh.classpath.ClassManagerImpl
     * for the fully functional class management package.
     *
    ...
    public Class classForName(String name) {
        ...
        // try scripted class
        if (clas == null) clas = loadSourceClass(name);
    ...
    public void cacheClassInfo(String name, Class value) {
        ...
        else absoluteNonClasses.put(name, NOVALUE);
    }

    /**
     * Associate a persistent generated class implementation with this interpreter. An associated
     * class will be used in lieu of generating bytecode when a scripted class of the same name is
     * encountered. When such a class is defined in the script it will cause the associated existing
    ...
    /**
     * Cache a resolved (possibly overloaded) method based on the argument types used to invoke it,
     * subject to classloader change. Static and Object methods are cached separately to support
    ...
    protected Method getResolvedMethod(
        ...
        if (method == null && !onlyStatic) method = (Method) resolvedObjectMethods.get(sk);
    ...
    /**
     * Set an external class loader. BeanShell will use this at the same point it would otherwise
     * use the plain Class.forName(). i.e. if no explicit classpath management is done from the
     * script (addClassPath(), setClassPath(), reloadClasses()) then BeanShell will only use the
     * supplied classloader. If additional classpath management is done then BeanShell will perform
     * that in addition to the supplied external classloader. However BeanShell is not currently
     * able to reload classes supplied through the external classloader.
     */
    ...
    /**
     * Flag the class name as being in the process of being defined. The class manager will not
     * attempt to load it.
     */
    /*
    ...
    */
    ...
    /**
     * This method is a temporary workaround used with definingClass. It is to be removed at some
     * point.
     */
