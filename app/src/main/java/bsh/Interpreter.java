...
public class Interpreter implements Runnable, ConsoleInterface, Serializable {
    ...
    */
    ...
    public Interpreter(
        ...
        if (namespace == null) this.globalNameSpace = new NameSpace(null, bcm, "global");
        else this.globalNameSpace = namespace;
        ...
        */
        if (!(getu("bsh") instanceof bsh.This)) initRootSystemObject();

        if (interactive) loadRCFiles();

        long t2 = System.currentTimeMillis();
        if (Interpreter.DEBUG) Interpreter.debug("初始化花了 " + (t2 - t1) + "ms");
    }
    ...
    /**
     * Construct a new interactive interpreter attached to the specified console using the specified
     * parent namespace.
     */
            ...
            try {
                ...
                if (result instanceof Class)
                    try {
                        ...
                        System.err.println("类 " + result + " 的主方法抛出了 " + o);
                    }
            } catch (FileNotFoundException e) {
                System.out.println("找不到文件: " + e);
            } catch (TargetError e) {
                System.out.println("脚本抛出异常: " + e);
                if (e.inNativeCode()) e.printStackTrace(DEBUG, System.err);
            } catch (EvalError e) {
                System.out.println("模拟错误: " + e);
            } catch (IOException e) {
                System.out.println("I/O 错误: " + e);
            }
        } else {
        ...
        */
        if (interactive)
            try {
                ...
                println("BeanShell " + VERSION + " - 由 Pat Niemeyer (pat@pat.net) 开发");
            }
        ...
        while (!eof) {
            try {
                ...
                if (get_jjtree().nodeArity() > 0) { // number of child nodes
                    ...
                    if (callstack.depth() > 1) throw new InterpreterError("调用堆在增长!: " + callstack);
                ...
                error("语法解析器错误: " + e.getMessage(DEBUG));
                ...
                if (interactive) error("模拟错误: " + e.toString());
                else error("模拟: " + e.getMessage());
    ...
    public Object source(String filename, NameSpace nameSpace)
        ...
        if (Interpreter.DEBUG) debug("正在从源执行文件: " + file);
    ...
    */
    public Object eval(Reader in, NameSpace nameSpace, String sourceFileInfo
        ...
        */
        Interpreter localInterpreter =
                new Interpreter(in, out, err, false, nameSpace, this, sourceFileInfo);
            ...
            try {
                ...
                if (localInterpreter.get_jjtree().nodeArity() > 0) {
                    ...
                    if (getSaveClasses()
                            ...
                            && !(node instanceof BSHPackageDeclaration)) continue;
                ...
                */
                ...
                final EvalError evalError =
                        new EvalError(
                                "执行的文件: " + sourceFileInfo + " 内部错误: " + e.getMessage(),
                                node,
                                callstack);
                ...
                e.reThrow("执行的文件: " + sourceFileInfo);
            } catch (EvalError e) {
                if (DEBUG) e.printStackTrace();
                // failsafe, set the Line as the origin of the error.
                if (e.getNode() == null) e.setNode(node);
                ...
                final EvalError evalError =
                        new EvalError(
                                "执行的文件: " + sourceFileInfo + " 未知错误: " + e.getMessage(),
                                node,
                                callstack);
                evalError.initCause(e);
                throw evalError;
                ...
                final EvalError evalError =
                        new EvalError(
                                "执行的文件: " + sourceFileInfo + " 标识符解析错误: " + e.getMessage(),
                                node,
                                callstack);
                evalError.initCause(e);
                throw evalError;
    ...
    public final void error(Object o) {
        if (console != null) console.error("// 错误: " + o + "\n");
        else {
            err.println("// 错误: " + o);
            err.flush();
        }
    ...
    */
    ...
    Object getu(String name) {
        try {
            ...
            throw new InterpreterError("取出: " + e);
        }
    }

    /**
     * Assign the value to the name. name may evaluate to anything assignable. e.g. a variable or
     * field.
     */
    ...
    void setu(String name, Object value) {
        try {
            ...
            throw new InterpreterError("放置: " + e);
        }
    }
        ...
        */
        ...
        try {
            ...
            if (lhs.type != LHS.VARIABLE)
                throw new EvalError("不能取消, 不是变量: " + name, SimpleNode.JAVACODE, new CallStack());
    ...
    void loadRCFiles() {
        try {
            ...
            if (Interpreter.DEBUG) debug("无法找到执行命令(rc)文件: " + e);
        }
    }
    ...
    /**
     * Get the class manager associated with this interpreter (the BshClassManager of this
     * interpreter's global namespace). This is primarily a convenience method.
     */
    ...
    /**
     * Specify the source of the text from which this interpreter is reading. Note: there is a
     * difference between what file the interrpeter is sourcing and from what file a method was
     * originally parsed. One file may call a method sourced from another file. See SimpleNode for
     * origination file info.
     *
    ...
    /**
     * Get the parent Interpreter of this interpreter, if any. Currently this relationship implies
     * the following: 1) Parent and child share a BshClassManager 2) Children indicate the parent's
     * source file information in error reporting. When created as part of a source() / eval() the
     * child also shares the parent's namespace. But that is not necessary in general.
     */
    ...
    /**
     * Get the prompt string defined by the getBshPrompt() method in the global namespace. This may
     * be from the getBshPrompt() command or may be defined by the user as with any other method.
