...
class BSHAllocationExpression extends SimpleNode {
    ...
    public Object eval(CallStack callstack, Interpreter interpreter) throws EvalError {
        ...
        if (type instanceof BSHAmbiguousName) {
            ...
            if (args instanceof BSHArguments)
                ...
                return objectArrayAllocation(
                        name, (BSHArrayDimensions) args, callstack, interpreter);
        } else
            return primitiveArrayAllocation(
                    (BSHPrimitiveType) type, (BSHArrayDimensions) args, callstack, interpreter);
    }
    ...
    private Object objectAllocation(
        ...
        if (hasBody) {
            ...
            else return constructWithClassBody(type, args, body, callstack, interpreter);
        } else return constructObject(type, args, callstack);
    }

    private Object constructObject(Class type, Object[] args, CallStack callstack)
            throws EvalError {
        Object obj;
        try {
        ...
        if (className.indexOf("$") == -1) return obj;
        ...
        if (instanceNameSpace != null && className.startsWith(instanceNameSpace.getName() + "$")) {
            try {
                ClassGenerator.getClassGenerator()
                        .setInstanceNameSpaceParent(obj, className, instanceNameSpace);
    ...
    */
    private Object constructWithClassBody(
            Class type, Object[] args, BSHBlock block, CallStack callstack, Interpreter interpreter)
            throws EvalError {
    ...
    private Object constructWithInterfaceBody(
            Class type, Object[] args, BSHBlock body, CallStack callstack, Interpreter interpreter)
            throws EvalError {
        NameSpace namespace = callstack.top();
    ...
    private Object arrayAllocation(
        ...
        */
        ...
        if (result != Primitive.VOID) return result;
        else return arrayNewInstance(type, dimensionsNode, callstack);
    }
     ...
     * [][] and the type of foo[0] is int[], etc. Arrays may also be specified with undefined
     * trailing dimensions - meaning that the lower order arrays are not allocated as objects. e.g.
     ...
     * type to specified allocated dimensions yielding an array of dimensionality base + specified
     * with the base dimensons unallocated. To create the "base" array type we simply create a
     * prototype, zero length in each dimension, array and use it to get its class (Actually, I
     * think there is a way we could do it with Class.forName() but I don't trust this). The code is
     * simpler than the explanation... see below.
     */
    private Object arrayNewInstance(
            Class type, BSHArrayDimensions dimensionsNode, CallStack callstack) throws EvalError {
        if (dimensionsNode.numUndefinedDims > 0) {
            Object proto =
                    Array.newInstance(type, new int[dimensionsNode.numUndefinedDims]); // zeros
            type = proto.getClass();
        }
