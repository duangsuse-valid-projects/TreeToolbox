...
public class Variable implements java.io.Serializable {
    ...
    public void setValue(Object value, int context) throws UtilEvalError {
        ...
        // check this.value
        if (hasModifier("final") && this.value != null) throw new UtilEvalError("最终的变量不能重声明.");

        this.value = value;

        if (this.value == null) this.value = Primitive.getDefaultValue(type);

        if (lhs != null) {
            this.value = lhs.assign(Primitive.unwrap(value), false);
            return;
        }
        ...
        if (type != null)
            this.value =
                    Types.castObject(
                            value, type, context == DECLARATION ? Types.CAST : Types.ASSIGNMENT);
    }
