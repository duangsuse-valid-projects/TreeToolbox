 ...
 * mechanism provides a standard collections based interface to the namespace as well as a
 * convenient way to export and view values of the namespace without the ordinary BeanShell
 * wrappers. Variables are maintained internally in the normal fashion to support meta-information
 * (such as variable type and visibility modifiers), but exported and imported in a synchronized
...
public class ExternalNameSpace extends NameSpace {
    ...
    public ExternalNameSpace(NameSpace parent, String name, Map externalMap) {
        ...
        if (externalMap == null) externalMap = new HashMap();
    ...
    /**
     * Set the external Map which to which this namespace synchronizes. The previous external map is
     * detached from this namespace. Previous map values are retained in the external map, but are
     * removed from the BeanShell namespace.
     */
    ...
    */
    protected Variable getVariableImpl(String name, boolean recurse) throws UtilEvalError {
        ...
        if (value == null) {
            ...
            // found.
            if (localVar == null) var = createVariable(name, null /*type*/, value, null);
            else var = localVar;
        }
    ...
    /**
     * Clear all variables, methods, and imports from this namespace and clear all values from the
     * external map (via Map clear()).
     */
    public void clear() {
        super.clear();
        externalMap.clear();
