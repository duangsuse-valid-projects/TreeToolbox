...
class BSHType extends SimpleNode implements BshClassManager.Listener {
    ...
    /**
     * Returns a class descriptor for this type. If the type is an ambiguous name (object type)
     * evaluation is attempted through the namespace in order to resolve imports. If it is not found
     * and the name is non-compound we assume the default package for the name.
     */
    public String getTypeDescriptor(
        ...
        if (node instanceof BSHPrimitiveType)
            ...
            if (clas != null) {
                ...
                else descriptor = "L" + defaultPackage.replace('.', '/') + "/" + clasName + ";";
            }
        }
    ...
    public Class getType(CallStack callstack, Interpreter interpreter) throws EvalError {
        ...
        else baseType = ((BSHAmbiguousName) node).toClass(callstack, interpreter);
