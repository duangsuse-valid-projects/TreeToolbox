 ...
 * belongs in LHS.java or a generic object wrapper class.
 *
 * @author Pat Niemeyer
 * @author Daniel Leuck
 */
...
class Reflect {
    /**
     * Invoke method on arbitrary object instance. invocation may be static (through the object
    ...
    /**
     * Invoke a method known to be static. No object instance is needed and there is no possibility
     * of the method being a bsh scripted method.
     */
    public static Object invokeStaticMethod(
        ...
        Interpreter.debug("调用静态方法");
        Method method = resolveExpectedJavaMethod(bcm, clas, null, methodName, args, true);
        return invokeMethod(method, null, args);
    }

    /**
     * Invoke the Java method on the specified object, performing needed type mappings on arguments
     * and return values.
     *
     * @param args may be null
     */
    static Object invokeMethod(Method method, Object object, Object[] args)
            throws ReflectError, InvocationTargetException {
        if (args == null) args = new Object[0];

        logInvokeMethod("调用方法 (进入): ", method, args);
        ...
        try {
            for (int i = 0; i < args.length; i++)
                tmpArgs[i] =
                        Types.castObject(args[i] /*rhs*/, types[i] /*lhsType*/, Types.ASSIGNMENT);
        } catch (UtilEvalError e) {
            throw new InterpreterError("方法调用的参数非法: " + e);
        }

        // unwrap any primitives
        tmpArgs = Primitive.unwrap(tmpArgs);

        logInvokeMethod("调用方法 (处理值以后): ", method, tmpArgs);
        ...
        try {
            ...
            throw new ReflectError(
                    "无法访问方法 "
                            + StringUtil.methodString(method.getName(), method.getParameterTypes())
                            + " 在 '"
    ...
    public static Object getIndex(Object array, int index) throws ReflectError, UtilTargetError {
        if (Interpreter.DEBUG) Interpreter.debug("获取索引: " + array + ", 索引=" + index);
        try {
    ...
    /**
     * Check for a field with the given name in a java object or scripted object if the field exists
     * fetch the value, if not check for a property value. If neither is found return
     * Primitive.VOID.
     */
    ...
    private static Object getFieldValue(
        ...
        try {
            ...
            throw new ReflectError("???" + fieldName + " 不是静态字段.");
        } catch (IllegalAccessException e) {
            throw new ReflectError("无法访问字段: " + fieldName);
        }
    }
    ...
    */
    protected static Field resolveJavaField(Class clas, String fieldName, boolean staticOnly)
    ...
    */
    protected static Field resolveExpectedJavaField(
            Class clas, String fieldName, boolean staticOnly) throws UtilEvalError, ReflectError {
        Field field;
        try {
            if (Capabilities.haveAccessibility()) field = findAccessibleField(clas, fieldName);
            else
                // Class getField() finds only public (and in interfaces, etc.)
                field = clas.getField(fieldName);
        } catch (NoSuchFieldException e) {
            throw new ReflectError("没有这个字段: " + fieldName);
        } catch (SecurityException e) {
            throw new UtilTargetError("寻找字段时发生安全异常: " + clas, e);
        }

        if (staticOnly && !Modifier.isStatic(field.getModifiers()))
            throw new UtilEvalError("获取不到势力化的字段: " + fieldName + " 在静态上下文中: " + clas.getName());

        return field;
    }

    /**
     * Used when accessibility capability is available to locate an occurrence of the field in the
     * most derived class or superclass and set its accessibility flag. Note that this method is not
     * needed in the simple non accessible case because we don't have to hunt for fields. Note that
     * classes may declare overlapping private fields, so the distinction about the most derived is
     * important. Java doesn't normally allow this kind of access (super won't show private
     * variables) so there is no real syntax for specifying which class scope to use...
     *
    ...
    */
    ...
    /**
     * This method wraps resolveJavaMethod() and expects a non-null method result. If the method is
     * not found it throws a descriptive ReflectError.
     */
    protected static Method resolveExpectedJavaMethod(
        ...
        if (object == Primitive.NULL)
            throw new UtilTargetError(new NullPointerException("尝试在空对象上调用方法: " + name));
    ...
    protected static Method resolveJavaMethod(
            ...
            throws UtilEvalError {
        if (clas == null) throw new InterpreterError("空的类");

        // Lookup previously cached method
        Method method = null;
        if (bcm == null) Interpreter.debug("未优化的resolveJavaMethod查询");
        else method = bcm.getResolvedMethod(clas, name, types, staticOnly);
        ...
        if (method == null) {
            ...
            try {
                ...
                throw new UtilTargetError("寻找方法时抛出安全异常: " + clas, e);
            }
    ...
    /**
     * Get the candidate methods by searching the class and interface graph of baseClass and resolve
     * the most specific.
     *
    ...
    private static Method findOverloadedMethod(
        ...
        if (Interpreter.DEBUG) Interpreter.debug("寻找最特殊的方法: " + methodName);
    ...
    /**
     * Climb the class and interface inheritance graph of the type and collect all methods matching
     * the specified name and criterion. If publicOnly is true then only public methods in *public*
    ...
    static Method[] getCandidateMethods(
        ...
        Vector candidates =
                gatherMethodsRecursive(baseClass, methodName, numArgs, publicOnly, null);
     ...
     * and we have to find the same public method in a parent class or interface.
     *
    ...
    private static Vector gatherMethodsRecursive(
        ...
        if (publicOnly) {
            ...
            addCandidates(
                    baseClass.getDeclaredMethods(), methodName, numArgs, publicOnly, candidates);
    ...
    private static Vector addCandidates(
        ...
        for (int i = 0; i < methods.length; i++) {
            ...
            if (m.getName().equals(methodName)
                    ...
                    && (!publicOnly || isPublic(m))) candidates.add(m);
        }
        return candidates;
    }
    ...
    static Object constructObject(Class clas, Object[] args)
            throws ReflectError, InvocationTargetException {
        if (clas.isInterface()) throw new ReflectError("不能创建接口的实例: " + clas);
        ...
        if (Interpreter.DEBUG) Interpreter.debug("寻找最特殊的架构器: " + clas);
        ...
        try {
            ...
            throw new ReflectError("类 " + clas + " 是抽象的 ");
        } catch (IllegalAccessException e) {
            throw new ReflectError("没有权限进行实例化." + "使用 setAccessibility(true) 来允许访问.");
        } catch (IllegalArgumentException e) {
            throw new ReflectError("参数个数错误");
        }
        if (obj == null) throw new ReflectError("无法架构对象");
    ...
    */
    ...
    /**
     * Find the best match for signature idealMatch. It is assumed that the methods array holds only
     * valid candidates (e.g. method name and number of args already matched). This method currently
    ...
    /**
     * Implement JLS 15.11.2 Return the index of the most specific arguments match or -1 if no match
     * is found. This method is used by both methods and constructors (which unfortunately don't
     * share a common interface for signature info).
     *
    ...
    */
    ...
    public static boolean hasObjectPropertySetter(Class clas, String propName) {
        ...
        for (int i = 0; i < methods.length; i++)
            if (methods[i].getName().equals(setterName)) return true;
        return false;
    }
    ...
    public static Object getObjectProperty(Object obj, String propName)
        ...
        if (method == null)
            throw new ReflectError("准确的getter出错: " + e1 + (e2 != null ? " : " + e2 : ""));

        try {
            return invokeMethod(method, obj, args);
        } catch (InvocationTargetException e) {
            throw new UtilEvalError("准确的访问器抛出异常: " + e.getTargetException());
        }
    }
    ...
    /**
     * Return a more human readable version of the type name. Specifically, array types are returned
     * with postfix "[]" dimensions. e.g. return "int []" for integer array instead of "class [I" as
    ...
    /**
     * Returns the base type of an array Class. throws ReflectError if the Class is not an array
     * class.
     */
    public static Class getArrayBaseType(Class arrayClass) throws ReflectError {
        if (!arrayClass.isArray()) throw new ReflectError("这个类不是数组.");

        return arrayClass.getComponentType();
    }

    /**
     * A command may be implemented as a compiled Java class containing one or more static invoke()
     * methods of the correct signature. The invoke() methods must accept two additional leading
     * arguments of the interpreter and callstack, respectively. e.g. invoke(interpreter, callstack,
     * ... ) This method adds the arguments and invokes the static method, returning the result.
     */
    public static Object invokeCompiledCommand(
        ...
        try {
            ...
            throw new UtilEvalError("已编译的命令中出错: " + e.getTargetException());
        } catch (ReflectError e) {
            throw new UtilEvalError("调用已编译命令时出错: " + e);
        }
    }

    private static void logInvokeMethod(String msg, Method method, Object[] args) {
        if (Interpreter.DEBUG) {
            Interpreter.debug(msg + method + " 使用参数:");
            for (int i = 0; i < args.length; i++) {
                ...
                Interpreter.debug(
                        "参数["
                                ...
                                + arg
                                + " 类型 = "
                                + (arg == null ? "<未知>" : arg.getClass()));
            }
        }
    }

    private static void checkFoundStaticMethod(Method method, boolean staticOnly, Class clas)
            throws UtilEvalError {
    ...
    private static ReflectError cantFindConstructor(Class clas, Class[] types) {
        if (types.length == 0) return new ReflectError("找不到默认架构器: " + clas);
        else
