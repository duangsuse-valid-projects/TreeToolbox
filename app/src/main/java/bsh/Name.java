 ...
 * "optimize" setting that defeats certain dynamic behavior we might be able to cache quite a bit.
 */
/*
...
class Name implements java.io.Serializable {
    ...
    private Object completeRound(String lastEvalName, String nextEvalName, Object returnObject) {
        if (returnObject == null) throw new InterpreterError("lastEvalName = " + lastEvalName);
    ...
    */
     ...
     * one component, but if the name is a classname it will consume all of the components necessary
     * to make the class identifier.
     */
    private Object consumeNextObjectField(
        ...
        */
        if ((evalBaseObject == null && !isCompound(evalName)) && !forceClass) {
            Object obj =
                    resolveThisFieldReference(callstack, namespace, interpreter, evalName, false);

            if (obj != Primitive.VOID) return completeRound(evalName, FINISHED, obj);
        }
        ...
        */
        if (evalBaseObject == null) {
            ...
            */
            ...
            for (; i <= countParts(evalName); i++) {
                ...
                if ((clas = namespace.getClass(className)) != null) break;
            }
        ...
        if ((evalBaseObject == null || evalBaseObject instanceof This)
            ...
            NameSpace targetNameSpace =
                    (evalBaseObject == null) ? namespace : ((This) evalBaseObject).namespace;
    ...
    Object resolveThisFieldReference(
        ...
        if (varName.equals("this")) {
            ...
            */
            ...
            if (classNameSpace != null) {
                if (isCompound(evalName)) result = classNameSpace.getThis(interpreter);
                else result = classNameSpace.getClassInstance();
            }
        ...
        */
        ...
        if (varName.equals("global")) obj = thisNameSpace.getGlobal(interpreter);

        if (obj == null && specialFieldsVisible) {
            if (varName.equals("namespace")) obj = thisNameSpace;
            else if (varName.equals("variables")) obj = thisNameSpace.getVariableNames();
    ...
    public synchronized Class toClass() throws ClassNotFoundException, UtilEvalError {
        ...
        if (evalName.equals("var")) return asClass = null;
    ...
    public synchronized LHS toLHS(CallStack callstack, Interpreter interpreter)
        ...
        try {
            while (evalName != null && isCompound(evalName)) {
                obj = consumeNextObjectField(callstack, interpreter, false /*forcclass*/, true);
            }
        } catch (UtilEvalError e) {
    ...
    /**
     * Invoke a locally declared method or a bsh command. If the method is not already declared in
     * the namespace then try to load it as a resource from the imported command path (e.g.
     * /bsh/commands)
     */
    /*
    ...
    */
    private Object invokeLocalMethod(
        ...
        if (meth != null) return meth.invoke(args, interpreter, callstack, callerInfo);
